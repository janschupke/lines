...
export class MigrationConfigLoader {
   ...
   */
  static async loadMigrationConfig(configPath: string = 'migrations/migrations.json'): Promise<Migration[]> {
    try {
      const response = await fetch(`/src/database/${configPath}`);
      if (!response.ok) {
        throw new Error(`Failed to load migration config: ${response.statusText}`);
      }
      ...
      throw new Error(`Migration configuration loading failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private static validateAndSortMigrations(migrations: Migration[]): Migration[] {
    if (!Array.isArray(migrations)) {
      throw new Error('Migration configuration must contain an array of migrations');
    }
    ...
    const validatedMigrations = migrations.map((migration, index) => {
      if (!migration.version || typeof migration.version !== 'number') {
        throw new Error(`Migration at index ${index} must have a valid version number`);
      }
      ...
      if (!migration.downFile || typeof migration.downFile !== 'string') {
        throw new Error(`Migration at index ${index} must have a valid downFile`);
      }
  ...
  static async loadMigrationsFromDirectory(): Promise<Migration[]> {
    try {
      ...
      throw new Error('Directory-based migration loading is not implemented for client-side environments');
    } catch (error) {
