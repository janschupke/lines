...
describe('MigrationLoader', () => {
  describe('loadMigrationFile', () => {
    it('should load high scores table migration successfully', async () => {
      const content = await MigrationLoader.loadMigrationFile('migrations/001_create_high_scores_table.sql');
      
      expect(content).toContain('CREATE TABLE IF NOT EXISTS high_scores');
      expect(content).toContain('CREATE INDEX IF NOT EXISTS idx_high_scores_score');
    ...
    it('should throw error for non-existent migration file', async () => {
      await expect(
        MigrationLoader.loadMigrationFile('migrations/999_nonexistent_migration.sql')
      ).rejects.toThrow('Migration file not found: migrations/999_nonexistent_migration.sql');
    });
  });
  ...
  describe('validateMigrationContent', () => {
    ...
    it('should reject whitespace-only content', () => {
      ...
      const isValid = MigrationLoader.validateMigrationContent(whitespaceContent);
      expect(isValid).toBe(false);
    });
