...
export class GamePhaseManager {
   ...
   */
  static handleLineDetection(board: Cell[][], toX: number, toY: number): MoveResult | null {
    ...
    const ballsToRemove: [number, number][] = Array.from(ballsToRemoveSet).map(key => {
      const [x, y] = key.split(',').map(Number);
      return [x, y];
    });
    
    // Remove balls
    const boardAfterRemoval = board.map(row => row.map(cell => ({ ...cell })));
  ...
  static handleIncomingBallConversion(board: Cell[][]): ConversionResult {
    ...
    const boardWithRealBalls = board.map(row => row.map(cell => ({
      ...cell,
    ...
    const afterBalls = this.placePreviewBalls(boardWithRealBalls, nextPreviewBalls);
   ...
   */
  private static placePreviewBalls(board: Cell[][], colors: BallColor[]): Cell[][] {
   ...
   */
  static validateMove(board: Cell[][], fromX: number, fromY: number, toX: number, toY: number): boolean {
