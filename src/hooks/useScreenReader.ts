...
export const useScreenReader = () => {
  ...
  const createLiveRegion = useCallback((priority: AnnouncementPriority = 'polite') => {
  ...
  const announceAssertive = useCallback((message: string, timeout = 1000) => {
    announce(message, { priority: 'assertive', timeout });
  }, [announce]);

  // Announce with polite priority (waits for current speech to finish)
  const announcePolite = useCallback((message: string, timeout = 1000) => {
  ...
  const announceNavigation = useCallback((destination: string) => {
    announce(`Navigated to ${destination}`, { priority: 'polite', timeout: 1500 });
  }, [announce]);

  // Utility for announcing modal state
  const announceModalState = useCallback((isOpen: boolean, modalName: string) => {
    if (isOpen) {
