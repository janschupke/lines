...
const mockEnv = { VITE_SUPABASE_URL: 'https://test.supabase.co', VITE_SUPABASE_ANON_KEY: 'anon', VITE_ENVIRONMENT: 'test' };
...
describe('ProductionDeploymentService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ProductionDeploymentService(mockSupabase, mockEnv, mockSchemaManager);
  });
  ...
  it('should throw if required env vars are missing', async () => {
    ...
    const serviceWithEnv = new ProductionDeploymentService(mockSupabase, env, mockSchemaManager);
