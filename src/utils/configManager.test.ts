...
describe('ConfigManager', () => {
  ...
  it('should add high score when score is higher in new session', () => {
    ...
    (manager as unknown as { config: { currentSessionDate: string; currentSessionScore: number } }).config.currentSessionDate = yesterday.toISOString();
    (manager as unknown as { config: { currentSessionScore: number } }).config.currentSessionScore = 50;
  ...
  it('should not add high score when score is lower and list is full', () => {
    ...
    (manager as unknown as { config: { highScores: typeof mockHighScores; currentSessionDate: string; currentSessionScore: number } }).config.highScores = mockHighScores;
    (manager as unknown as { config: { currentSessionDate: string; currentSessionScore: number } }).config.currentSessionDate = new Date().toISOString();
    (manager as unknown as { config: { currentSessionScore: number } }).config.currentSessionScore = 50;
