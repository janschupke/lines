...
describe("sanitization", () => {
  describe("sanitizePlayerName", () => {
    ...
    it("should remove HTML tags", () => {
      expect(sanitizePlayerName('<script>alert("xss")</script>')).toBe(
        'alert("xss")',
      );
      expect(sanitizePlayerName('<img src="x" onerror="alert(1)">')).toBe(
    ...
    it.skip("should remove JavaScript injection patterns", () => {
      ...
      expect(sanitizePlayerName('vbscript:msgbox("test")')).toBe(
        'msgbox("test")',
      );
      expect(
        sanitizePlayerName("data:text/html,<script>alert(1)</script>"),
      ).toBe("text/html,scriptalert(1)/script");
    });
    ...
    it("should handle edge cases", () => {
      ...
      expect(sanitizePlayerName("🍆")).toBe("🍆");
      expect(sanitizePlayerName("Test<script>alert(1)</script>Name")).toBe(
        "Testalert(1)Name",
      );
    });
  });
  ...
  describe("getPlayerNameError", () => {
    ...
    it("should return error for names that are too long", () => {
      const longName = "a".repeat(51);
      expect(getPlayerNameError(longName)).toBe(
        "Name must be 50 characters or less",
      );
    });

    it("should return error for names with invalid characters", () => {
      expect(getPlayerNameError("<script>alert(1)</script>")).toBe(
        "Name contains invalid characters",
      );
      expect(getPlayerNameError("javascript:alert(1)")).toBe(
        "Name contains invalid characters",
      );
      expect(getPlayerNameError("Hello<World>")).toBe(
        "Name contains invalid characters",
      );
      expect(getPlayerNameError('Test"Quote')).toBe(
        "Name contains invalid characters",
      );
    });
